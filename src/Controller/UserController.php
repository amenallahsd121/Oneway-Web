<?php

namespace App\Controller;

use Twilio\Rest\Client;
use ReCaptcha\ReCaptcha;
use App\Form\SmsFormType;
use App\Form\CodeFormType;
use App\Form\PassFormType;
use App\Form\UserFormType;
use App\Entity\Utilisateur;
use App\Form\LoginFormType;
use Psr\Log\LoggerInterface;
use App\Form\NewpassFormType;
use Symfony\Component\Mime\Email;
use Symfony\Component\Mailer\Mailer;
use Symfony\Component\Mailer\Transport;
use App\Repository\UtilisateurRepository;
use Doctrine\Persistence\ManagerRegistry;
use Symfony\Component\Console\Output\Output;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\Mailer\MailerInterface;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;
use Symfony\Component\HttpFoundation\RedirectResponse;
use Symfony\Component\HttpFoundation\Session\SessionInterface;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\Mailer\Exception\TransportExceptionInterface;
use Symfony\Component\DependencyInjection\Loader\Configurator\session;



class UserController extends AbstractController
{
    


    public function backbase(SessionInterface $session): Response
{
    $username = $session->get('username');
    
    return $this->render('backbase.html.twig', [
        'username' => $username,
    ]);
}





    #[Route('/front', name: 'signup_user')]
    public function ajoutUserAction(Request $request, ManagerRegistry $Doctrine, SessionInterface $session)
    {
        $session->start();
        $username = $_SESSION['username'] ?? null;
        if ($username != null) {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("aff_user");
        }
        $user = new Utilisateur();
        $form = $this->createForm(UserFormType::class, $user);
        $form->handleRequest($request);
        $email = $form["email"]->getData();
        $pass = $form["password"]->getData();
        if ($form->isSubmitted() && $form->isValid()) {
            $user->setType("Client");
            $user->setNbPoint(0);
            $user->setCode(1);
            $em = $Doctrine->getmanager();
            $em->persist($user);
            $em->flush();
            //  return $this->redirectToRoute('signup_user');
        }

        return $this->render("user/frontSignup.html.twig", [
            "form" => $form->createView()
        ]);
    }

    #[Route('/afficher', name: 'aff_user')]
    public function afficherUsers(ManagerRegistry $Doctrine, Request $request, UtilisateurRepository $userRepository, SessionInterface $session)
    {
        $session->start();
        $t = $_SESSION['user_type']  ;
        $username = $_SESSION['username'] ?? null;
        if ($username === null) {

            echo "<script>alert('Login first');</script>";
            return $this->redirectToRoute("check_login");
        } else if($t == "Admin")
        {
           // echo "<script>alert('this user is  $username ');</script>";
        }else {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("front_edit");
        }

        /*$em = $Doctrine->getManager();
        $user = $em->getRepository(Utilisateur::class)->findBy(array('type' => "Client"));
        return $this->render("user/userView.html.twig",array('user' =>$user));*/
        $em = $Doctrine->getManager();
        $userRepository = $em->getRepository(Utilisateur::class);
        $searchQuery = $request->query->get('q');

        if ($searchQuery) {
            $users = $userRepository->searchUsers($searchQuery);
        } else {
            $users = $userRepository->findBy(['type' => 'Client']);
        }
        // add sorting functionality
        $sort = $request->query->get('sort');
        if ($sort == 'asc') {
            usort($users, function ($a, $b) {
                return strcmp($a->getName(), $b->getName());
            });
        } elseif ($sort == 'desc') {
            usort($users, function ($a, $b) {
                return strcmp($b->getName(), $a->getName());
            });
        }

        return $this->render('user/userView.html.twig', [
            'user' => $users,
            'searchQuery' => $searchQuery,
            'username' => $username,
        ]);
    }
    #[Route('/logout', name: 'user_logout')]
    public function logout(SessionInterface $session): RedirectResponse
    {
        $session->start();
        echo "<script>alert('wow');</script>";
        $_SESSION['username'] = null;
        $_SESSION['user_id'] =   null;
        $_SESSION['user_type'] =  null;
       


        return $this->redirectToRoute("check_login");
    }


    #[Route('/user/{id}/delete', name: 'user_delete')]
    public function delete(Utilisateur $user, ManagerRegistry $Doctrine): RedirectResponse
    {
        $em = $Doctrine->getManager();
        $em->remove($user);
        $em->flush();
        if ($user->gettype() == "Client") {
            return $this->redirectToRoute("aff_user");
        } else
            return $this->redirectToRoute("aff_admin");
    }
    #[Route('/user/{id}/edit', name: 'user_edit')]
    public function edit(Utilisateur $user, Request $request, ManagerRegistry $Doctrine)
    {
        $form = $this->createForm(UserFormType::class, $user);
        $form->handleRequest($request);
        if ($form->isSubmitted()) {
            $em = $Doctrine->getManager();
            $em->persist($user);
            $em->flush();
            if ($user->gettype() == "Client") {
                return $this->redirectToRoute("aff_user");
            } else
                return $this->redirectToRoute("aff_admin");
        }
        return $this->render("user/editUser.html.twig", [
            "form" => $form->createView()
        ]);
    }

    #[Route('/check', name: 'check_login')]
    public function checklogin(Request $request, ManagerRegistry $Doctrine, LoggerInterface $logger, SessionInterface $session)
    {
        $session->start();
        $username = $_SESSION['username'] ?? null;
        if ($username != null) {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("aff_user");
        }
        $client = new Utilisateur();
        $formm = $this->createForm(LoginFormType::class, $client);
        $formt = $this->createForm(UserFormType::class, $client);
        $formm->handleRequest($request);
        if ($formm->isSubmitted()) {

            $em = $Doctrine->getManager();
            $users = $em->getRepository(Utilisateur::class)->findall();
            $name = $formm["name"]->getData();
            $pass = $formm["password"]->getData();
            foreach ($users as $e) {
                $nameb = $e->getName();
                $passwordb = $e->getPassword();
                $type = $e->gettype();
                $cod = $e->getCode();
                $idd = $e->getId();
                if ($nameb == $name) {
                    if ($passwordb == $pass) {
                        $key = "6LcNYcUlAAAAAAtejxmZ0O3KJwF8U84mSBcLBlWH";
                        $recaptcha = new ReCaptcha($key);
                        $recaptcha_response = $request->get('g-recaptcha-response');

                        if (!$recaptcha_response) {
                            // If the reCAPTCHA validation fails, display an error message
                            $this->addFlash('error', 'The reCAPTCHA was not submitted.');
                            return $this->redirectToRoute('check_login');
                        }
                        $session->start();

                        // Set a session variable
                        $_SESSION['username'] = $name;
                        $_SESSION['user_id'] =   $idd;
                        $_SESSION['user_type'] =   $type;


                        //return new Response('Session variable set.');
                        if ($cod == 1) {

                            if ($type == "Admin") {
                                echo "<script>alert('existe');</script>";
                                return $this->redirectToRoute('aff_user');
                            } else
                            
                            return $this->redirectToRoute('front_edit');
                        } else {
                            echo "<script>alert('this user is banned');</script>";
                        }
                    }
                }
            }

            $logger->info($name);

            /*$em = $Doctrine->getManager();
            $em->persist($clients);
            $em->flush();*/
        }
        return $this->render("user/loginpage.html.twig", [
            "formL" => $formm->createView()
        ]);
    }
    #[Route('/show', name: 'aff_admin')]
    public function afficherAdmins(ManagerRegistry $Doctrine, Request $request, UtilisateurRepository $userRepository)
    {

        $em = $Doctrine->getManager();
        $userRepository = $em->getRepository(Utilisateur::class);
        $searchQuery = $request->query->get('q');

        if ($searchQuery) {
            $users = $userRepository->searchAdmins($searchQuery);
        } else {
            $users = $userRepository->findBy(['type' => 'Admin']);
        }
        // add sorting functionality
        $sort = $request->query->get('sort');
        if ($sort == 'asc') {
            usort($users, function ($a, $b) {
                return strcmp($a->getName(), $b->getName());
            });
        } elseif ($sort == 'desc') {
            usort($users, function ($a, $b) {
                return strcmp($b->getName(), $a->getName());
            });
        }

        return $this->render('user/adminView.html.twig', [
            'user' => $users,
            'searchQuery' => $searchQuery,
            
        ]);
    }









    #[Route('back', name: 'add_admin')]
    public function ajoutAdmin(Request $request, ManagerRegistry $Doctrine)
    {
        $user = new Utilisateur();
        $form = $this->createForm(UserFormType::class, $user);
        $form->handleRequest($request);
        $email = $form["email"]->getData();
        $pass = $form["password"]->getData();
        if ($form->isSubmitted()) {

            $user->setType("Admin");
            $user->setNbPoint(0);
            $user->setCode(1);
            $em = $Doctrine->getmanager();
            $em->persist($user);
            $em->flush();
            return $this->redirectToRoute('aff_admin');
        }

        return $this->render("user/addAdmin.html.twig", [
            "form" => $form->createView()
        ]);
    }











    #[Route('/pass', name: 'check_pass')]
    public function password(Request $request, MailerInterface $mailer, LoggerInterface $logger, ManagerRegistry $Doctrine, SessionInterface $session)
    {
        $session->start();
        $username = $_SESSION['username'] ?? null;
        if ($username != null) {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("aff_user");
        }
        $client = new Utilisateur();
        $formm = $this->createForm(PassFormType::class, $client);
        $formm->handleRequest($request);
        if ($formm->isSubmitted()) {
            $random = random_int(10000, 99999);
            $emailc = $formm["email"]->getData();
            if (filter_var($emailc, FILTER_VALIDATE_EMAIL)) {
                $em = $Doctrine->getManager();
                $users = $em->getRepository(Utilisateur::class)->findall();
                foreach ($users as $e) {
                    $emaill = $e->getEmail();
                    if ($emailc == $emaill) {
                        $pcode = $e->getCode();
                        $e->setCode($random);
                        $em = $Doctrine->getmanager();
                        $em->persist($e);
                        $em->flush();
                        ////////////////////////////////////////////////////////////////

                        // Create a Transport object
                        $transport = Transport::fromDsn('smtp://oneway.noreplay@gmail.com:ruhgodtwnckgjezy@smtp.gmail.com:465');

                        // Create a Mailer object
                        $mailer = new Mailer($transport);

                        // Create an Email object
                        $email = (new Email());

                        // Set the "From address"
                        $email->from('oneway.noreplay@gmail.com');

                        // Set the "To address"
                        $email->to($emailc);

                        // Set a "subject"
                        $email->subject('change password !');

                        // Set the plain-text "Body"
                        $email->text('Test Recu Mail.');

                        // Set HTML "Body"
                        $email->html('
        <div style="border:2px solid green; padding:20px; font-family: Arial, sans-serif;">
          <img src="data:image/png;base64," alt="Logo" style="max-width: 200px; float: right;">
          <h1 style="color:#006600; margin-top:0;">Bonjour monsieur</h1>  
          <p style="font-size:18px;">Voici votre code  : .</p>
          <p style="font-size:18px;"> ' . $random . '.</p>
          <div class="d-flex justify-content-center">
          <span class="bi bi-truck" style="font-size: 4rem;"></span>
        </div>
        
          <p style="font-size:18px;">Pour plus d\'informations, n\'hésitez pas à nous contacter.</p>
          <a href="#" style="display:inline-block; margin-top:20px; padding:10px 20px; background-color:#006600; color:#fff; text-decoration:none; border-radius:5px;">Nous contacter</a>
        </div>
      ');



                        // Sending email with status
                        try {
                            // Send email
                            $mailer->send($email);
                        } catch (TransportExceptionInterface $e) {
                        }

                        /*return $this->render("user/entercode.html.twig",[
                            "formL" =>$formt->createView()
                        ]);*/
                        return $this->redirectToRoute("check_code", ['pcode' => $pcode]);
                    }
                }
            } else {
                echo "L'adresse e-mail n'est pas valide";
            }






            //$logger->info($email );
            /*$em = $Doctrine->getManager();
            $em->persist($clients);
            $em->flush();*/
        }
        return $this->render("user/sofyenpass.html.twig", [
            "formL" => $formm->createView()
        ]);
    }














    #[Route('/sms', name: 'check_sms')]
    public function sms(Request $request, MailerInterface $mailer, ManagerRegistry $Doctrine, SessionInterface $session)
    {
        $session->start();
        $username = $_SESSION['username'] ?? null;
        if ($username != null) {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("aff_user");
        }
        //$client = new Utilisateur();
        $formm = $this->createForm(SmsFormType::class);
        $formm->handleRequest($request);
        if ($formm->isSubmitted()) {
            $em = $Doctrine->getManager();
            $users = $em->getRepository(Utilisateur::class)->findBy(array('type' => "Client"));
            foreach ($users as $e) {
                $pcode = $e->getCode();
                $random = random_int(10000, 99999);
                $e->setCode($random);
                $em->persist($e);
                $em->flush();
                $sms = $formm["phone"]->getData();

                $twilionumber = "+16073885845";

                $client = new Client('AC954042c1468214186635e317b0d82cea', '831eb539737a03327f83cb791f8782c0');
                $client->messages->create(
                    // Where to send a text message (your cell phone?)
                    $sms,
                    array(
                        'from' => $twilionumber,
                        'body' => 'your reset code : ' . $random . ' '
                    )
                );
                return $this->redirectToRoute("check_code", ['pcode' => $pcode]);
            }
        }
        return $this->render("user/smspage..html.twig", [
            "formL" => $formm->createView()
        ]);
    }











    #[Route('/code/{pcode}', name: 'check_code')]
    public function enterCode(Request $request, ManagerRegistry $Doctrine, LoggerInterface $logger, $pcode, SessionInterface $session)
    {
        $session->start();
        $username = $_SESSION['username'] ?? null;
        if ($username != null) {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("aff_user");
        }
        $client = new Utilisateur();
        $formm = $this->createForm(CodeFormType::class, $client);
        $formt = $this->createForm(PassFormType::class, $client);
        $formm->handleRequest($request);
        $formt->handleRequest($request);
        if ($formm->isSubmitted()) {
            $em = $Doctrine->getManager();
            $users = $em->getRepository(Utilisateur::class)->findall();
            $codde = $formm["code"]->getData();
            foreach ($users as $e) {
                $codee = $e->getCode();
                if ($codee == $codde) {
                    $e->setCode($pcode);
                    $em = $Doctrine->getmanager();
                    $em->persist($e);
                    $em->flush();
                    return $this->redirectToRoute("check_npass", ['id' => $e->getId()]);
                }
            }

            echo "<script>alert('Wrong Code');</script>";

            $logger->info($codde);
        }
        return $this->render("user/entercode.html.twig", [
            "formL" => $formm->createView()
        ]);
    }






    #[Route('/newpass/{id}', name: 'check_npass')]
    public function newpassword(Utilisateur $user, Request $request, ManagerRegistry $Doctrine, LoggerInterface $logger, SessionInterface $session)
    {
        $session->start();
        $username = $_SESSION['username'] ?? null;
        if ($username != null) {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("aff_user");
        }
        $formm = $this->createForm(NewpassFormType::class);
        $formm->handleRequest($request);
        if ($formm->isSubmitted()) {
            $em = $Doctrine->getManager();
            $users = $em->getRepository(Utilisateur::class)->findall();
            $pass = $formm["password"]->getData();
            $Rpass = $formm["Rpassword"]->getData();
            if ($pass == $Rpass) {
                foreach ($users as $e) {

                    $passwordb = $e->getPassword();
                    if ($passwordb == $pass) {
                        echo "<script>alert('password exist');</script>";

                        return $this->render("user/newpass.html.twig", [
                            "formL" => $formm->createView()
                        ]);
                    } else {
                        $user->setPassword($pass);
                        $em = $Doctrine->getmanager();
                        $em->persist($user);
                        $em->flush();
                        return $this->redirectToRoute("check_login");
                    }
                }
            } else
                echo "<script>alert('not the same ');</script>";
        }
        return $this->render("user/newpass.html.twig", [
            "formL" => $formm->createView()
        ]);
    }
    #[Route('/user/{id}/ban', name: 'user_ban')]

    public function ban(Utilisateur $user, Request $request, ManagerRegistry $Doctrine, SessionInterface $session)
    {
        /*$session->start();
        $username = $_SESSION['username'] ?? null;
        if ($username != null) {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("aff_user");
        } */
        if ($user->getCode() == 1) {
            $user->setCode(2);
            $em = $Doctrine->getmanager();
            $em->persist($user);
            $em->flush();
        } else if ($user->getCode() == 2) {
            $user->setCode(1);
            $em = $Doctrine->getmanager();
            $em->persist($user);
            $em->flush();
        }


        return $this->redirectToRoute("aff_user");
    }

    #[Route('/frontedit', name: 'front_edit')]
    public function frontedit(Request $request, ManagerRegistry $Doctrine, SessionInterface $session)
    {
        $session->start();
        $id = $_SESSION['user_id'];
        $t = $_SESSION['user_type']  ;
        $username = $_SESSION['username'] ?? null;
        if ($username === null) {

            echo "<script>alert('Login first');</script>";
            return $this->redirectToRoute("check_login");
        } else if($t == "Client")
        {
            //echo "<script>alert('this user is  $username ');</script>";
        }else {
            echo "<script>alert('Logout first');</script>";
            return $this->redirectToRoute("aff_user");
        }
        $em = $Doctrine->getManager();
        $user = $em->getRepository(Utilisateur::class)->find($id);


        $form = $this->createForm(UserFormType::class, $user);
        $form->handleRequest($request);
        if ($form->isSubmitted()) {
            $em = $Doctrine->getManager();
            $em->persist($user);
            $em->flush();
            return $this->redirectToRoute("front_edit");
        }
        return $this->render("user/frontpage.html.twig", [
            "form" => $form->createView(),
            'namee' => $user->getName(),
            'emaill' => $user->getEmail(),
        ]);
    }
}
